...
type CacheManager interface {
	...
	Raw() *redis.Client
	Store(ctx context.Context, key string, item *Item) error
}
...
func (c *cacheManager) Store(ctx context.Context, key string, item *Item) error {
	ctx, span := tracing.Start(ctx)
	defer span.End()

	return c.Set(ctx, key, item.Value(), item.TTL())
}
